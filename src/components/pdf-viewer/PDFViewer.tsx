...
const PDFViewer = ({ fileUrl }) => {
	...
	useEffect(() => {
		...
		const handleFindEvent = (event) => {
			console.log("Find event data:", event);
			if (!event.matchesCount) {
				alert("No results found!");
			}
		};

		eventBus?.on("updatefindmatchescount", handleFindEvent);

		return () => {
			eventBus?.off("updatefindmatchescount", handleFindEvent);
		};
	}, []);

	const handleFind = (isPrevious = false) => {
		if (searchTerm) {
			const eventBus = pdfViewerRef.current?.eventBus;
			if (eventBus) {
				eventBus.dispatch("find", {
					source: null, // Sumber event
					type: "find",
					query: searchTerm, // Kata yang dicari
					caseSensitive: false, // Tidak peka huruf besar/kecil
					highlightAll: true, // Highlight semua hasil
					findPrevious: isPrevious, // Navigasi ke hasil sebelumnya jika true
				});

				console.log(pdfViewerRef.current?.findController?._matchesCountTotal);
				console.log(pdfViewerRef.current?.findController?.state);
			}
		}
	};
	...
	return (
			...
			<>
				<div id="outerContainer">
					...
					<div id="mainContainer">
						...
						<div ref={viewerContainer} id="viewerContainer" tabIndex={0}>
		...
		// 				value={searchTerm}
		// 			/>
		// 			<button onClick={() => handleFind(true)}>Find Previous</button>
		// 			<button onClick={() => handleFind(false)}>Find Next</button>
		// 		</div>
		// 	</div>
		// 	<div
		// 		id="viewerContainer"
